...
class GitHubAPI:
    """Lightweight wrapper around the GitHub REST API with pagination helpers."""
    ...
    def iterate(self, path: str, params: Optional[Dict[str, Any]] = None) -> Iterable[Dict[str, Any]]:
        ...
        while next_url:
            ...
            if response.status_code >= 400:
                self._log.error(
                    "GitHub API pagination failed: %s %s", response.status_code, response.text
                )
                response.raise_for_status()
