...
from core_backup.config import GitHubOrganizationExports, GitHubRepositoryOptions
...
def backup_organization_metadata(
    api: GitHubAPI,
    organization: Optional[str],
    exports: GitHubOrganizationExports,
    metadata_root: Path,
) -> Dict[str, int]:
    if not organization:
        return {}
    ...
    counts: Dict[str, int] = {}
    org_dir = metadata_root / organization
    org_dir.mkdir(parents=True, exist_ok=True)

    if exports.members:
        members = list(api.iterate(f"orgs/{organization}/members"))
        _write_json(org_dir / "members.json", members)
        counts["org_members"] = len(members)

    if exports.teams:
        teams = list(api.iterate(f"orgs/{organization}/teams"))
